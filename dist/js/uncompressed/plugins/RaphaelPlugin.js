...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push(function () {
  ...
  _getTransform = function _getTransform(t, rec) {
    var s = t.matrix,
        min = 0.000001,
        a = s.a,
        b = s.b,
  ...
  _parseAngle = function _parseAngle(v, d) {
    ...
    init: function init(target, value, tween) {
      ...
      for (p in value) {
        ...
        s = target.attr(p); //Some of these properties are in place in order to conform with the standard PropTweens in TweenPlugins so that overwriting and roundProps occur properly. For example, f and r may seem unnecessary here, but they enable other functionality.
    ...
    set: function set(v) {
      ...
      this._target.attr(this._props); //apply transform values like x, y, scaleX, scaleY, rotation, skewX, or skewY. We do these after looping through all the PropTweens because those are where the changes are made to scaleX/scaleY/rotation/skewX/skewY/x/y.
      ...
      if (this._transform) {
        ...
        var ang = pt.rotation,
            ...
            min = 0.000001,
            pxl = this._pxl,
            pyl = this._pyl; //some browsers have a hard time with very small values like 2.4492935982947064e-16 (notice the "e-" towards the end) and would render the object slightly off. So we round to 0 in these cases for both b and c. The conditional logic here is faster than calling Math.abs().
...
//# sourceMappingURL=RaphaelPlugin.js.map
